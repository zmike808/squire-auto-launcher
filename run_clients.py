...
SQUIRE_JAR_PATH = Path("./jar/squire-launcher.jar")
## CHANGE THESE TWO VARIABLES TO YOUR OWN JAVA AND SQUIRE.JAR LOCATION!!!!!!!!!!!!
...
def run_clients():
    with Path("accounts.txt").open() as f:
        ...
        parser.add_argument("--skip", type=bool, default=True, help="automatically skip auth and nightly")
        ...
        print(f"base launching arguements {main_arg}")
        if args.skip:
        ...
        for client in clients:
            if client.startswith("#"):
                continue
            launch_arg = main_arg
            print(f"to split {client} predicted result {client.split(',')}")
            accinfo = client.split(",")
            print(accinfo)
            if len(accinfo) == 0 or len(accinfo) == 1:
                run_arg = f"{launch_arg}"
                if len(accinfo) == 1:
                    coloncount = len([x for x in accinfo[0] if x == ":"])
                    if coloncount >= 1:
                        run_arg = f"{launch_arg} --proxy={accinfo[0]}"
                print(f"proxy only : {run_arg}")
                proc: subprocess.Popen = subprocess.Popen(run_arg)#, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, start_new_session=True)
                procs.append(proc)
                time.sleep(1)
                continue
            username = accinfo[0]
